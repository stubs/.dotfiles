...
M.workon_curator = function()
    ...
    require('telescope.builtin').find_files({ prompt_title = '~ Curator ~', cwd = '/opt/dotdash/dataops/curator/' })
end

M.workon_aqueduct = function()
    vim.cmd [[:tabnew | tcd /opt/dotdash/dataops/aqueduct/]]
    require('telescope.builtin').find_files({ prompt_title = '~ Aqueduct ~', cwd = '/opt/dotdash/dataops/aqueduct/' })
end
